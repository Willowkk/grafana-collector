...
- 用模拟网络与时钟、真实 CLI / engine / SQLite / Parquet 和 XLSX 输出验证：完整成功和合法空结果清理；普通失败、部分失败、中断、认证异常、输出失败、浏览器关闭异常保留；恢复后成功可清理。
- 持续采集覆盖 SIGINT / SIGTERM、任务取消、duration 短于首轮轮询、轮询之间的尾窗、慢查询跨截止时刻，以及最新轮成功但历史显示区间有缺口的情况。
- 中断轮次已经保存的点照常输出；时钟回拨后的恢复不会裁剪已保存的较晚数据；清理检查覆盖全部已保存范围。
- 验证新请求不创建 raw 文件、不持久化响应正文，旧 raw 文件与旧库记录不被改写，新导出不暴露旧 raw 路径；历史 v2 样本与新旧来源审计均兼容。
- 用启动替身核对有窗口/无窗口 Chrome 均收到三个参数，viewport、时区、语言和 profile 配置仍保持原值；此项不是实际浏览器环境验收。

0.4.0 wheel 构建成功，并安装到独立临时目录。在仓库之外核对导入路径和版本，验证 CLI 帮助仅列出 `login/inspect/fetch/watch`、已移除的 `export` 返回参数错误，并通过 wheel 中的读取器读到历史 v2 样本面板 166 的 52 个点。

## 历史验证记录（0.1.x–0.3.0）

以下为对应版本当时的原始验证记录，保留其中的 raw、SQLite、离线导出行为和测试数量作为历史证据。它们不代表 0.4.0 仍提供独立离线导出命令，也不代表新请求仍保存 raw。历史验收样本保持原样。

历史验证日期：2026-09-17；Mac、Google Chrome、Python 3.12.14。独立安装另外使用 Python 3.11.16。目标为 SDKv2 大盘 `inyUtsTSk`，版本 319；固定历史窗口为北京时间 2026-09-14 15:00–21:00，文件系统 `cfs_echo_hl_qa_fs`，其余筛选见示例配置。
